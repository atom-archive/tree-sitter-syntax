...
class TreeSitterLanguageMode {
  ...
  toggleLineCommentsForBufferRows (start, end) {
    ...
    const shouldUncomment = rows.every(([_row, line]) => !/\S/.test() || line.search(/\S/) === line.indexOf(commentStartString))
    ...
    if (shouldUncomment) {
      ...
      rows.forEach(([row, line]) => {
        ...
        if (firstChar !== -1) {
          this.buffer.insert(Point(row, firstChar), commentStartString)
        }
      })
    }
  }
