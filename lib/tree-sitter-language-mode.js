...
class TreeSitterLanguageMode {
  ...
  toggleLineCommentsForBufferRows (start, end) {
    const {
      commentStartString,
      commentEndString
    } = this.commentStringsForPosition()

    if (!commentStartString) return

    const rows = Array.apply(null, Array(end - start + 1)).map((_, i) => [start + i, this.buffer.lineForRow(start + i)])

    const {commentedRows, uncommentedRows} = rows.reduce((agg, [row, line]) => {
      if (/\S/.test()) {
        // If we have a comment string at the beginning of the line
        if (line.search(/\S/) === line.indexOf(commentStartString.trim())) {
          agg.commentedRows.push([row, line])
        } else {
          agg.uncommentedRows.push([row, line])
        }
      }

      return agg
    }, {commentedRows: [], uncommentedRows: []})

    const shouldUncomment = uncommentedRows.length === 0 && commentedRows.length > 0

    if (shouldUncomment) {
      commentedRows.forEach(([row, line]) => {
        let commentStartRange = [line.indexOf(commentStartString), commentStartString.length]
        if (commentStartRange[0] === -1) {
          const trimmedStartString = commentStartString.trim()
          commentStartRange = [line.indexOf(trimmedStartString), trimmedStartString.length]
        }

        if (commentStartRange[0] === -1) return

        this.buffer.delete([[row, commentStartRange[0]], [row, commentStartRange[0] + commentStartRange[1]]])
      })
    } else {
      const minIndentLevel = uncommentedRows.reduce((agg, [row, line]) => {
        const indentLevel = this.indentLevelForLine(line);

        return indentLevel < agg ? indentLevel : agg
      }, Infinity)

      const indentColumn = Number.isFinite(minIndentLevel) ? minIndentLevel * this.tabLength : 0;

      uncommentedRows.forEach(([row]) => this.buffer.insert(Point(row, indentColumn), commentStartString))
    }
  }
