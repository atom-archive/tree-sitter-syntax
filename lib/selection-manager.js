...
export default class SelectionManager {
  constructor ({editor, document}) {
    ...
    atom.commands.add(
      ...
      {
        'syntax-tree:select-up': this.selectUp.bind(this),
        'syntax-tree:select-down': this.selectDown.bind(this),
        'syntax-tree:select-left': this.selectLeft.bind(this),
        'syntax-tree:select-right': this.selectRight.bind(this),
        'syntax-tree:print-tree': this.printTree.bind(this),
        'syntax-tree:toggle-debug': this.toggleDebug.bind(this),
        'syntax-tree:reparse': this.reparse.bind(this)
      }
    )
  }
  ...
  printTree () {
    if (this.editor.getSelectedText() === "") {
      ...
      for (let range of this.editor.getSelectedBufferRanges()) {
        ...
        const node = this.document.rootNode.descendantForRange(start, end - 1)
  ...
  toggleDebug () {
    if (this.document.getDebugger()) {
      this.document.setDebugger(null)
    } else {
      this.document.setDebugger((msg, params, type) => {
        if (type === 'parse') {
  ...
  updatedSelectedNodes (fn) {
    ...
    const newRanges = selectedRanges.map((range, i) => {
      ...
      let node = this.document.rootNode.descendantForRange(currentStart, currentEnd - 1)
